...
date: today
date-format: long
abstract: "This study utilizes a decade of Toronto Police Service data spanning from 2014 to 2024 to examine the dynamics of bicycle thefts, focusing specifically on monthly and hourly variations, as well as the influence of different locations. Analysis reveals that the theft rates are higher in summer months and during late evening hours under the apartment buildings. This research highlights critical insights into urban crime patterns, providing valuable information for enhancing security measures and planning urban safety strategies effectively."
format: pdf
...
In addition to the statistical methods applied in our study, we utilized a specifically developed API to predict bike theft incidents based on several variables including the hour of the day, month of the year, premises type, and the cost of the bike. This API, built using the `plumber` and `rstanarm` packages, leverages a Bayesian logistic regression model trained on historical data from 2014 to 2024, as sourced from the Toronto Police Service. The API's predictive functionality allows us to dynamically assess the risk of bike theft by integrating real-time data inputs, offering a practical application for our research findings.

The API processes input variables such as the occurrence hour and month, type of premises, and bike cost to estimate the probability of a theft. This is done by calculating the odds of theft using the model's posterior samples for each input factor. The resulting predictions provide insights into the patterns of theft across different times and locations, demonstrating how these factors influence bike theft risk. This predictive tool not only validates our analytical models but also serves as a bridge between theoretical research and practical, real-world application, enhancing the utility of our findings for urban safety and crime prevention strategies.
